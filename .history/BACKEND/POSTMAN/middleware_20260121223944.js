...
//log aur authentication ke liye alag alag middleware banaye ja sakte hain
//app.use() ka matlab hai ki ye middleware har request ke liye chalega chahe woh get ho post ho delete ho put ho patch ho etc isse hame 
// app.get ya saare http methods ke liye alag alag middleware banane ki jarurat nahi haii isse jada code lkhane ki jarurat nahi haii
//MIDDLEWARE KO SECURITY HANDLE CHIZ NAII HAII BAS CODE KO CLEAN AUR REUSABLE BANANE KE LIYE USE KARTE HAIN

app.use((req,res,next)=>{
    // const now=new Date();
    console.log(`Request received at ${Date.now} for ${req.method} ${req.originalUrl}`);
    next();
})
app.get("/user",(req,res)=>{
            ...
            // next();
            res.send("Welcome to the User Page with Logging GET Middleware");
}
)

app.post("/user",(req,res)=>{
         
            res.send("Welcome to the User Page with Logging POST Middleware");
})

app.delete("/user",(req,res)=>{
            res.send("Welcome to the User Page with Logging DELETE Middleware");
})
